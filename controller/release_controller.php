...
if (isset($_POST['save_item'])){
    ...
    $category_val = $conn->query("SELECT cat_id,cat_name,item_code,unit FROM tbl_item where item_id='$item_id' LIMIT 1") or die ($conn->error);
    $cat_row = $category_val->fetch_array();
    $cat_id =$cat_row['cat_id'];
    $cat_name =$cat_row['cat_name'];
    $item_code =$cat_row['item_code'];
    $unit_group = $cat_row['unit_group'];
    $get_qty_ind = $cat_row['qty'];
    $unit = $_POST['unit'];
    if($unit_group == $unit){
        $qty_rls_ind = $_POST['qty'] * $get_qty_ind;
    }else{
        $qty_rls_ind = $_POST['qty'];
    }
   ...
   $conn->query("INSERT INTO tbl_itemtrans(trans_type,emp_id,emp_name,item_id,item_name,item_code,cat_id,cat_name,description,qty_rls_ind,release_qty,unit,trans_date,remarks,status,user_name,user_id) 
    VALUES('Released','$emp_id','$emp_name','$item_id','".addslashes($item_name)."','$item_code','$cat_id','$cat_name','$description','$qty_rls_ind','$release_qty','$unit','$trans_date','$remarks','Active','$user_name','$user_id')");
    ...
    if($conn->error){
        ...
        $save_autrail = $conn->query("INSERT INTO tbl_audittrail (trans_type,descs,action,user_id,user_name)
                VALUES('release transaction','$item_emp','Release','$user_id','$user_name')");
...
if(isset($_GET['delete'])){
    ...
    WHERE id=$id ") or die ($conn->error);
        ...
        VALUES('$id','release transaction','$emp_name','delete','$user_id','$user_name')");
...
if(isset($_POST['update_item'])){
    ...
    $user_id=$_SESSION['id'];
        ...
        $conn->query("UPDATE tbl_itemtrans SET emp_id='$emp_id',emp_name='$emp_name',item_id='$item_id',item_name='".addslashes($item_name)."',cat_id='$cat_id',
                    cat_name='$cat_name',description='$description',user_id='$user_id',user_name='$user_name',qty_rls_ind='$qty_rls_ind',
        ...
        if($conn->error){
            ...
            $item_emp = $item_name . ' / '. $emp_name;
            $save_autrail = $conn->query("INSERT INTO tbl_audittrail (trans_id,trans_type,descs,action,user_id,user_name)
                            VALUES('$id','release transaction','$item_emp','update','$user_id','$user_name')");
...
class Item {
    ...
    public function getStocksAvail(){
        $sqlQuery = "SELECT i.item_id,i.description,i.item_name,i.unit,total_qty/i.qty as qty_package, t.avail_stocks 
            FROM tbl_item as i JOIN (SELECT id,item_id,cat_name,item_name,sum(qty_ind) as total_qty,
            sum(total) as total,sum(release_qty) as release_qty, sum(qty_ind-release_qty)
            as avail_stocks,  date_created FROM tbl_itemtrans WHERE status='Active' 
            GROUP BY item_id) as t WHERE t.item_id = i.item_id";

        $result= mysqli_query($this->dbConnect, $sqlQuery);	
        $itemHTML = '';
        while( $item = mysqli_fetch_assoc($result)) {
			$itemHTML .= '<option value="'.$item["item_id"].'#>'.$item["item_name"].'#>'.$item["avail_stocks"].'#>'.$item["unit"].'#>'.$item['description'].'#>'. $item['unit_group'].'">'.$item["item_name"]. '</option>';	
		}
        return $itemHTML;
    }
}
